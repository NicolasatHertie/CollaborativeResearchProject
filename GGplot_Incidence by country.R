...
ggplot2::ggplot(Merged, aes(Incidence, country)) + geom_line() + theme_bw()

# Look at incidence over time
ggplot(aes(x = year, y = Incidence), data = Merged) + geom_line()

# Look at the relation between Incidence and Health Care Spending per capita (+ geom_smooth() for adding a line )
